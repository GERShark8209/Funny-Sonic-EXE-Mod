...
class PlayState extends MusicBeatState
{
	...
	var parappaFreestyleSounds:Array<String> = ['punch', 'block', 'chop', 'kick'];

	public var freestyleActive:Bool = false;
	...
	public function startCountdown():Void
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			if (parappaSong)
			{
				...
				{ // just to give it time to finish the transition
	...
	override public function update(elapsed:Float)
	{
		...
		if (parappaSong)
		{
			iconP1.y = healthBar.y
				+ (healthBar.height * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01))
	...
	function missSoundEffect(press:Bool = true)
	{
		...
		{
			case 'bf':
				FlxG.sound.play(Paths.soundRandom('funny parappa/parappamiss', 1, 2), FlxG.random.float(0.9, 1));
			default:
				if (!press)
	...
	function freestyleHit(noteData:Int):Void
	{
		if (cpuControlled)
			return;
		...
		trace(parappaFreestyleSounds[noteData]);
		// trace('funny parappa/' + parappaFreestyleSounds[noteData]);

		var animToPlay:String = singAnimations[noteData];
		FlxG.sound.play(Paths.soundRandom('funny parappa/freestyle/' + parappaFreestyleSounds[noteData], 1, 4), 1);
